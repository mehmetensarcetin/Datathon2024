...
# Age Binning
age_binner = KBinsDiscretizer(n_bins=4, encode='ordinal', strategy='uniform')
train['Age_Binned'] = age_binner.fit_transform(train[['Age']])
test['Age_Binned'] = age_binner.transform(test[['Age']])
...
# Set up the parameter grid for RandomizedSearchCV with more iterations
param_grid = {
    'depth': [4, 6, 8, 10],
...
model = CatBoostRegressor(loss_function='RMSE', cat_features=categorical_features, random_state=42, verbose=0, early_stopping_rounds=50)

# Implementing TimeSeriesSplit for cross-validation
tscv = TimeSeriesSplit(n_splits=5)

# Set up RandomizedSearchCV
random_search = RandomizedSearchCV(estimator=model, param_distributions=param_grid, 
                                   n_iter=100, cv=tscv, verbose=1, random_state=42, n_jobs=-1)
...
# Prepare submission file for CatBoost
...
# Optional Ensemble Model: Combine CatBoost, LightGBM, and XGBoost for stacking
stacking_model = StackingRegressor(
    ...
    final_estimator=CatBoostRegressor(cat_features=categorical_features, random_state=42, verbose=0)
)
...
# Prepare submission file for the stacking model
submission_stack = test_processed[['id']].copy()
submission_stack['Degerlendirme Puani'] = y_pred_stack
